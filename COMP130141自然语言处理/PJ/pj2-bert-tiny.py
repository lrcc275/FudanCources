...
import numpy as np
batch = [4,8,16,32,64]
lrs = [2e-5,4e-5,6e-5,8e-5,1e-4,2e-4]
...
plt.title("In a bert-tiny for MNLI,accuracy with different batch and lr")
plt.xlabel("lr")
plt.ylabel("batch")
plt.show()
...
plt.title("In a bert-tiny for MRPC,accuracy with different batch and lr")
plt.xlabel("lr")
plt.ylabel("batch")
plt.show()
